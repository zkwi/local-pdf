...
<p align="center">
  <b>Convert PDF to Word and Markdown entirely in your browser.</b><br>
...
`public/sitemap.xml` and `SITE_URL` in `src/ui/SeoContent.tsx`. Change them when hosting under another domain.
...
- `public/samples/demo.pdf` (the "try a sample" file) is generated by `scripts/make-demo-pdf.py`, which needs PyMuPDF locally.
- `docs/social-card.png` / `public/og.png` come from `scripts/make-social-card.py` (also PyMuPDF).
